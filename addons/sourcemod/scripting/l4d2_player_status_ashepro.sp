...
int    g_iPlayerGrab,g_iPlayerImpact, g_iPlayerDeath, g_iPlayerDown;
ConVar g_hPlayerGrab, g_hPlayerImpact, g_hPlayerDeath, g_hPlayerDown;

public Plugin myinfo = 
{
	name 			= "l4d2_player_status",
	author 			= "阿蛇改 豆瓣酱な ",
	description 	= "幸存者各种提示.增加牛牛撞人",
...
public void OnPluginStart()
{
	...
	g_hPlayerImpact	= CreateConVar("l4d2_enabled_player_Impact", "1", "启用幸存者被创飞提示? 0=禁用, 1=启用.", FCVAR_NOTIFY);
	...
	g_hPlayerImpact.AddChangeHook(IsConVarChanged);
	g_hPlayerDeath.AddChangeHook(IsConVarChanged);
	g_hPlayerDown.AddChangeHook(IsConVarChanged);
	
	AutoExecConfig(true, "l4d2_player_status_ashepro");//生成指定文件名的CFG.
}
...
void IsGetCvars()
{
	...
	g_iPlayerImpact = g_hPlayerImpact.IntValue;
...
public void Event_Impact(Event event, const char[] name ,bool Broadcast)
{
	...
	if (!g_iPlayerImpact || g_bShowPromptVariable)
		return;
	...
	if(IsValidEntity(victim))
		...
		{
			case 1:
				CPrintToChatAll("{green}★★★★★ {blue}%s{olive}被牛牛创飞咯", GetTrueName(victim));//聊天窗提示.
			case 0:
				CPrintToChatAll("{green}★★★★★ {olive}牛牛抓着{blue}%s{olive}把{blue}%s{olive}创飞咯", GetTrueName(carry),GetTrueName(victim));//聊天窗提示.
		}
}
