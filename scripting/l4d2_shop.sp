...
#include <pan0s>
#include <shop_cv>
#include <entity_prop_stocks>  //快捷键
...
{
    name = "l4d2_Shop (Points and Gift System)",
    author = "pan0s, Drakcol, AXIS_ASAKI",
...
// ====================================================================================================
	...
	RegConsoleCmd("sm_b", HandleCmdOpenShop);
	...
	//RegConsoleCmd("sm_zs", HandleCmdSuicide);
	...
	cvar_suicide_cmd_on 					= CreateConVar("shop_suicide_cmd_on","0","Can survivor suicide by !kill? 0=OFF, 1=On",CVAR_FLAGS, true, 0.0, true, 1.0);
...
void CreateDBTable()
{
	...
	if (db != null)
	{
		DBResultSet rsShop = SQL_Query(db, "CREATE TABLE IF NOT EXISTS Shop(steamId TEXT, points INTEGER, Name TEXT, PRIMARY KEY (steamId))");
		char isSucceed[255];
		...
		else
		{
			...
			if (SQL_Execute(hShopQuery))
			{
				...
				else //INSERT
				{
					...
					if ((hInsertStmt = SQL_PrepareQuery(db, "INSERT INTO Shop(steamId, points, Name) VALUES(?,?,?)", error, sizeof(error))) == INVALID_HANDLE)
					{
					...
					else
					{
						...
						hInsertStmt.BindString(2, playerName, false); 
...
int SaveToDB(int client)
{
	...
	if (db != null)
	{
		if(client && IsValidClient(client) && !IsFakeClient(client))
		{
			...
			char steamId[32];
			GetClientAuthId(client, AuthId_Steam2, steamId, sizeof(steamId));
			if ((hUpdateShopStmt = SQL_PrepareQuery(db, "UPDATE Shop SET points = ?, Name = ? WHERE steamId = ?", error, sizeof(error))) == null)
			{
			...
			else
			{
				...
				hUpdateShopStmt.BindString(1, playerName, false);
				hUpdateShopStmt.BindString(2, steamId, false);				
...
public void OnClientPostAdminCheck(int client)
{
    ...
    if(IsValidClient(client) && !IsFakeClient(client) && cvar_db_on.BoolValue)
    {
		...
		Shop[client]= CreateTimer(0.3, Shopmenu, client, TIMER_REPEAT);  //ashe
		LogAction(-1, -1, "cratetimer:Cl[%d]",client);//ashe
		if(IsValidHandle(Shop[client])) LogAction(-1, -1, "cratetimersuccess:Cl[%d]",client);//ashe
    }
}

public void OnClientDisconnect(int client)
{
	if(IsValidHandle(Shop[client]))//ashe
	{	
		KillTimer(Shop[client]);//ashe
		LogAction(-1, -1, "killtime:Cl[%d]",client);//ashe
	}
...
public Action Shopmenu(Handle Timer, int client)//ashe
{
	if(client && IsValidClient(client) && !IsFakeClient(client))
	{
		if(GetClientButtons(client)==1536) 
		{
			Menu menu = new Menu(HandleMenuBuy);
			ShowShopMenu(client, menu, SHOP_MAIN);
		}
	}
	return Plugin_Continue;
}
...
public Action Event_PlayerDeath(Event event, char[] event_name, bool dontBroadcast)
{
	...
	if (IsSurvivor(attacker) && IsInfected(client))
	{
		...
		AddPoints(attacker, points);//击杀特感加点
	}
...
public Action KillPoints(Event event, char[] event_name, bool dontBroadcast)
{
	...
	if (IsSurvivor(client))
	{
		...
		if (g_iKilledInfectedCount[client] >= cvar_earn_infected_num.IntValue)
		{
			...
			AddPoints(client, points);//杀小ss加点
