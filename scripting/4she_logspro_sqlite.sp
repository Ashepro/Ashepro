...
#define PLUGIN_NAME				"logspro_sqlite"
#define PLUGIN_AUTHOR			"阿蛇"
#define PLUGIN_VERSION			"1.0"
#define PLUGIN_DESCRIPTION		"本地数据库日志保存各种、顺便记录时长"
#define PLUGIN_URL				""
...
//阿蛇新增-----------------------------------------------------------------------------------
	...
	LoadTranslations("a2s_chapters.phrases");//加载翻译文件
	LoadTranslations("a2s_missions.phrases");//加载翻译文件
	/* Register CVars */
...
public void OnMapStart() {
	...
	//g_smMissionMap.GetString(map, displayname, sizeof(displayname));
	//fmt_Translate(displayname, displayname, sizeof(displayname), 0, map);
	//fmt_Translate(map, ChapterName, sizeof(ChapterName), 0, "");
...
void ChangeMapName() {
	...
	Format(MapName, sizeof(MapName), "%T", MapName, 0);	// 当前地图所在的任务译名 (ex. c1m1_hotel => 1:酒店)
	Format(MissionName, sizeof(MissionName), "%T", MissionName, 0);	// 当前地图的章节译名 (ex. L4D2C1 => 死亡中心)
